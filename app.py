...
def runner():
    ...
    args = {
        "image": "/Users/jannathshaik/Desktop/python/projects/courseprojects/ocr-project/uplodedimage.jpg",
        "reference":"/Users/jannathshaik/Desktop/python/projects/courseprojects/ocr-project/micr.jpg"
    }
    ...
    for (i, c) in enumerate(groupCnts):
        ...
        # accept the contour region as a grouping of characters regardless of size
    ...
    # plt_imshow("Check OCR", image)
    img_final_pil = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))

    st.image(img_final_pil, caption='Final Image', use_column_width=True)
    
st.set_page_config(
    page_title="Bank Cheque MICR Detection App",
)
...
custom_footer = """
    ...
    <div class="custom-footer">
        Made by Khirthana, Jannath and Mithil 
    </div>
"""

# Streamlit app layout
st.title('Bank Cheque MICR Detection')
st.sidebar.title('Bank Cheque MICR Detection')
# Sidebar for uploading an image
uploaded_file = st.sidebar.file_uploader('Upload an image', type=['jpg', 'jpeg', 'png'])
st.sidebar.info("Please upload an image")

if uploaded_file is None:
    st.header('About Bank Cheque MICR Detection App')

    st.write("Welcome to our Bank Cheque MICR Detection app!")
    st.write(
        "Our app utilizes computer vision techniques to automatically detect and interpret MICR characters from scanned bank cheques.")

    st.write("## How it Works:")
    st.write(
        "Simply upload a scanned cheque image, and our app swiftly processes it, providing reliable MICR data interpretation for seamless banking operations.")

    st.markdown(
        """
        For more information on MICR please visit (https://en.wikipedia.org/wiki/Magnetic_ink_character_recognition)
        """
    )
    st.sidebar.markdown(custom_footer, unsafe_allow_html=True)
